...
def critical_thickness(row, nominal, cfg):
    """Critical (minimum required) wall thickness in mm.

    Primary method: ASME B31.8 required thickness
        t_req = P*D / (2*S*F*E*T)   with P in psi, D in inch -> converted to mm.
    A pipe already thinner than t_req is beyond its normative minimum, so it is
    flagged as end-of-life: t_crit is capped just under nominal so remaining life
    collapses to ~0 and the pipe reads as Critical.
    Fallback: legacy fraction of nominal when P or D is unavailable.
    """
    method=cfg.get("CRIT_METHOD","LEGACY")
    if method=="ASME_B31.8":
        try:
            P=float(row[cfg["PRESSURE_COL"]])          # psi
            D=float(row[cfg["DIAMETER_COL"]])          # inch (nominal OD proxy)
            S=cfg["SMYS_PSI"]; F=cfg["DESIGN_FACTOR"]
            E=cfg["JOINT_FACTOR"]; T=cfg["TEMP_DERATE"]
            t_req_mm=(P*D)/(2.0*S*F*E*T)*25.4          # inch -> mm
            if not np.isfinite(t_req_mm) or t_req_mm<=0:
                raise ValueError
            # cap so an under-designed pipe becomes end-of-life, never impossible
            return float(min(t_req_mm, 0.98*nominal))
        except Exception:
            pass
    frac=cfg.get("T_CRIT_FRAC_LEGACY", cfg.get("T_CRIT_FRAC",0.70))
    return frac*nominal
...
def predict_one(user_inputs, b):
    ...
    # ── Single model: predict wall thickness across every tree ───────────────
    ...
    tcrit=critical_thickness(row, nominal, cfg)
    cr=np.maximum((nominal-tree_thk)/age,1e-6)
    tree_rl=np.clip((tree_thk-tcrit)/cr,0,cap)          # remaining life per tree
    ...
    # ── Class is READ from the calculated remaining life (API 570 bins) ──────
    bins=cfg["RISK_BINS"]; labels=cfg["RISK_LABELS"]
    risk_class=str(pd.cut([rl_median],bins=bins,labels=labels)[0])

    # ── Binary decision derives from the class (Critical/Monitor -> Inspect) ─
    inspect_labels=set(cfg.get("INSPECT_LABELS",["Critical","Monitor"]))
    decision="Inspect" if risk_class in inspect_labels else "OK"
    # safety margin: force Inspect if predicted life is short
    if rl_median<cfg["SAFETY_MARGIN"]: decision="Inspect"

    # ── Confidence from agreement of the trees, weighted by coverage ─────────
    spread=(rl_high-rl_low)/max(cap,1e-6)               # 0 = tight, 1 = very wide
    agreement=1.0-min(spread,1.0)
    confidence=0.5+(agreement-0.5)*coverage

    return {"decision":decision,"risk_class":risk_class,"confidence":confidence,
            "rl_median":rl_median,
...
def predict_batch(dframe,b):
    """Vectorised batch prediction: the whole table goes through the forest once."""
    cfg=b["config"]; cap=cfg["RL_CAP"]; feats=b["features"]
    n=len(dframe)

    # ── Build the feature frame, filling anything missing with medians ───────
    X=pd.DataFrame(index=dframe.index)
    provided=np.zeros(n,dtype=float)
    for c in feats:
        if c in dframe.columns:
            col=dframe[c]
            provided+=col.notna().to_numpy().astype(float)
            X[c]=col.fillna(b["medians"][c])
        else:
            X[c]=b["medians"][c]
    X=X[feats]
    coverage=provided/len(feats)

    # ── One transform, then every tree predicts on the whole table ───────────
    pre=b["regressor"].named_steps["pre"]; forest=b["regressor"].named_steps["rf"]
    Xt=pre.transform(X)
    tree_thk=np.vstack([t.predict(Xt) for t in forest.estimators_])   # (n_trees, n)

    nominal=pd.to_numeric(X[b["nominal_col"]],errors="coerce").to_numpy(dtype=float)
    age=np.maximum(pd.to_numeric(X[b["age_col"]],errors="coerce").to_numpy(dtype=float),1e-6)

    # ── Critical thickness, vectorised (ASME B31.8 with legacy fallback) ─────
    tcrit=np.full(n,np.nan)
    if cfg.get("CRIT_METHOD")=="ASME_B31.8":
        try:
            P=pd.to_numeric(X[cfg["PRESSURE_COL"]],errors="coerce").to_numpy(dtype=float)
            D=pd.to_numeric(X[cfg["DIAMETER_COL"]],errors="coerce").to_numpy(dtype=float)
            S=cfg["SMYS_PSI"]; F=cfg["DESIGN_FACTOR"]
            E=cfg["JOINT_FACTOR"]; T=cfg["TEMP_DERATE"]
            t_req=(P*D)/(2.0*S*F*E*T)*25.4
            tcrit=np.minimum(t_req,0.98*nominal)
        except Exception:
            pass
    frac=cfg.get("T_CRIT_FRAC_LEGACY",cfg.get("T_CRIT_FRAC",0.70))
    bad=~np.isfinite(tcrit)|(tcrit<=0)
    tcrit[bad]=frac*nominal[bad]

    # ── Remaining life across the forest ─────────────────────────────────────
    cr=np.maximum((nominal[None,:]-tree_thk)/age[None,:],1e-6)
    tree_rl=np.clip((tree_thk-tcrit[None,:])/cr,0,cap)
    rl_med=np.median(tree_rl,axis=0)
    rl_lo=np.percentile(tree_rl,10,axis=0)
    rl_hi=np.minimum(np.percentile(tree_rl,90,axis=0),cap)
    capped=rl_med>=cap

    # ── Class from the calculated life, then the binary decision ────────────
    labels=cfg["RISK_LABELS"]
    risk=pd.cut(rl_med,bins=cfg["RISK_BINS"],labels=labels).astype(str)
    inspect=set(cfg.get("INSPECT_LABELS",["Critical","Monitor"]))
    decision=np.where(pd.Series(risk).isin(inspect).to_numpy()|
                      (rl_med<cfg["SAFETY_MARGIN"]),"Inspect","OK")

    spread=(rl_hi-rl_lo)/max(cap,1e-6)
    confidence=0.5+((1.0-np.minimum(spread,1.0))-0.5)*coverage

    keep=dframe.copy()
    keep.insert(0,"Decision",decision)
    keep.insert(1,"Remaining life (yr)",
                np.where(capped,f"{cap:.0f}+",np.round(rl_med,1).astype(str)))
    keep.insert(2,"Risk class",risk)
    keep.insert(3,"Range",[f"{a:.0f} to {b_:.0f}" for a,b_ in zip(rl_lo,rl_hi)])
    keep.insert(4,"Confidence",(confidence*100).round(0).astype(int).astype(str)+"%")
    keep["_sort"]=rl_med
    return keep
